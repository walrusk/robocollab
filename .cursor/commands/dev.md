...
1. Run `scripts/start.sh` to check readiness.
...
3. Do not include steps for running any `scripts/` commands. You will run those behind the scenes as needed.
...
3. Run `scripts/branch.sh {name}` to create a new branch. Use the same name as the plan file but with dashes, e.g. `scripts/branch.sh 001-add-tailwind`.
...
1. Run `scripts/commit.sh "commit message"` to stage and commit all changes.
2. Run `scripts/pr.sh` to push the branch and create a PR on GitHub. `scripts/pr.sh` accepts the same arguments as `gh pr create`. Include a short PR description with bullet points if applicable.
