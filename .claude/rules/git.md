...
3. Read-only git commands (e.g. `git status`, `git diff`, `git log`, `git rev-parse`, `git branch`) are fine to run directly — still via `rtk`, e.g. `rtk git status`.
4. Never force push, reset, rebase, or amend unless the user explicitly asks for it.
5. Never run `git commit --no-verify` or otherwise skip hooks.
6. Never modify git config.
7. If you need a git operation not covered by a script and not listed as always-safe above, stop and ask the user before running it.
